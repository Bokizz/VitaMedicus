...
class Appointment(models.Model):
    ...
    patient = models.ForeignKey('accounts.User', 
    ...
    service = models.ForeignKey('hospitals.Service', 
                                on_delete = models.CASCADE,
                                null = True,
                                blank = True)
    department = models.ForeignKey('hospitals.Department', 
                                   on_delete = models.CASCADE)
    ...
    def __str__(self):
        return f"Термин со времетраење од {self.start_time} часот до {self.end_time} часот на ден {self.date} во болница {self.hospital.name} кај Доктор {self.doctor.user.first_name} {self.doctor.user.last_name}"
